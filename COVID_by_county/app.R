...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            tags$br(),
            tags$p(
                "These charts track new cases and deaths in the state and county
            ...
            tags$p(
                "It's hard to know how to interpret these numbers since there are
...
server <- function(input, output) {
    ...
    output$text_data <- renderText({
        ...
        total_state_deaths <-
            current_state$`sum(deaths)`[nrow(current_state)]
        ...
        current_state <- current_state %>%
            ...
            summarise(sum(cases))
        total_state_cases <-
            current_state$`sum(cases)`[nrow(current_state)]
        ...
        current_county <- current_county %>%
            ...
            summarise(sum(deaths))
        total_county_deaths <-
            current_county$`sum(deaths)`[nrow(current_county)]
        ...
        current_county <- current_county %>%
            ...
            summarise(sum(cases))
        total_county_cases <-
            current_county$`sum(cases)`[nrow(current_county)]
